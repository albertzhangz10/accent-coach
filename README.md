...
│   └── page.tsx
├── components/                 # 网页版组件
│   ├── Header.tsx              # 顶部导航栏（含品牌名与设置入口）
│   ├── SettingsPanel.tsx       # 设置下拉面板（语言切换 + 发音者选择）
│   ├── Recorder.tsx            # 录音、播放、评分按钮
│   └── ScoreDisplay.tsx        # 评分环、指标、重点词、教练点评
├── lib/
│   ├── i18n.tsx                # 国际化上下文（React Context + localStorage）
│   ├── translations/           # 翻译文件
│   │   ├── en.ts               # 英文界面字符串
│   │   └── zh.ts               # 中文界面字符串
